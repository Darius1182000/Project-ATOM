...
async function handleMessage(message) {
  ...
  if (command === 'debug') {
    ...
    sendMessage(
      message.channel,
      `**Manager properties:** ${managerProps.slice(0, 10).join(', ')}\n**Manager methods:** ${managerMethods
        .slice(0, 10)
  ...
  if (command === 'volume') {
    ...
    if (isNaN(volume) || volume < 0 || volume > 150)
      return sendMessage(message.channel, `Current volume: **${player.volume}%**\nUsage: \`.volume <0-150>\``);
  ...
  if (command === 'health' || command === 'checkyt') {
    const manager = message.client.manager;
    const testUrls = ['ytsearch:test audio', 'ytsearch:never gonna give you up'];
