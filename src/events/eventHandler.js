...
function setupEventHandlers(client) {
    ...
    // Compact error formatter for cleaner logs
    const summarizeError = (err) => {
        if (!err) return 'Unknown error';
        if (typeof err === 'string') return err;
        const name = err.name || 'Error';
        const msg = err.message || (err.reason || '').toString() || String(err);
        const code = err.code || err.errno || err.status || err.statusCode;
        return `${name}: ${msg}${code ? ` (code: ${code})` : ''}`;
    };

    // Ensure node-level 'error' events don’t bubble as uncaught
    const attachNodeListeners = (node) => {
        if (!node || !node.on) return;
        const flag = '__errorListenerAttached';
        if (node[flag]) return;
        node[flag] = true;

        node.on('error', (error) => {
            const id = node.options?.id || node.identifier || `${node.options?.host}:${node.options?.port}`;
            console.warn(`❌ Node "${id}" internal error -> ${summarizeError(error)}`);
        });
    };

    // Attach to existing nodes (important for early errors before 'nodeCreate')
    const nodesMap = manager?.nodeManager?.nodes ?? new Map();
    nodesMap.forEach(attachNodeListeners);

    // Listen on NodeManager itself to prevent ERR_UNHANDLED_ERROR from bubbling
    manager.nodeManager?.on?.('error', (node, error) => {
        const id = node?.options?.id || node?.identifier || `${node?.options?.host}:${node?.options?.port}`;
        console.warn(`🎚️ NodeManager error for "${id}": ${summarizeError(error)}`);
    });
    ...
    manager.on('nodeDisconnect', (node, reason) => {
        console.log(`❌ Lavalink node "${node.identifier}" disconnected: ${summarizeError(reason) || String(reason)}`);
    });

    manager.on('nodeError', (node, error) => {
        console.error(`❌ Lavalink node "${node.identifier}" encountered an error: ${summarizeError(error)}`);
    });
    ...
    manager.on('error', (error) => {
        console.error('❌ LavalinkManager error:', summarizeError(error));
    });

    manager.on('nodeCreate', (node) => {
        // Attach error listener for newly created nodes
